...
export function buildSeoDescription(keyword: string): string {
  const d = `${keyword}: IPTV UK TV explains internet TV, IPTV apps, and subscription options—plus links to setup guides.`;
  if (d.length <= 160) return d;
  return `${d.slice(0, 157)}…`;
}

export function buildSeoParagraph(keyword: string): string {
  return `This page is a short introduction to ${keyword} in the context of internet television (IPTV). For step-by-step installation on Fire TV, Smart TV, Android, PC, and more, open our setup guides. To compare IPTV subscription plans, visit the IPTV UK TV store.`;
}
